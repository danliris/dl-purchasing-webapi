...
var QuantityCorrectionManager = require("dl-module").managers.purchasing.UnitPaymentQuantityCorrectionNoteManager;
...
function getRouter() {
    ...
    var getManager = (user) => {
        return db.get()
            .then((db) => {
                return Promise.resolve(new QuantityCorrectionManager(db, user));
            });
    };
    ...
    router.get("/", passport, function (request, response, next) {
        ...
        getManager(user)
            .then((manager) => {
                ...
                return correctionManager.getMonitoringKoreksi(query);
            })
            .then(docs => {
            ...
            .then((result) => {
                if ((request.headers.accept || '').toString().indexOf("application/xls") < 0) {
                    ...
                    correctionManager.getXls(result, query)
                        .then(xls => {
