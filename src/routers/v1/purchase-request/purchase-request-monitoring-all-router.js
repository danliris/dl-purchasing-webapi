...
function getRouter() {
    ...
    router.get('/', passport, (request, response, next) => {
        db.get().then(db => {
            ...
            manager.getDataPRMonitoringAll(unitId, categoryId, budgetId, PRNo, dateFrom, dateTo, state, offset, createdBy)
                .then(docs => {
                    if ((request.headers.accept || '').toString().indexOf("application/xls") < 0) {
                        ...
                        for (var purchaseRequest of docs) {
                            for (var item of purchaseRequest.items) {
                                ...
                                var _item = {
                                    ...
                                    "Tanggal Diminta Datang Eksternal": moment(new Date(purchaseRequest.poEks.expectedDeliveryDate)).add(offset, 'h').format(dateFormat),
                                    "Status": status,
                                    "User": purchaseRequest.createdBy
                        ...
                        var options = {
                            ...
                            "Tanggal Diminta Datang Eksternal": "string",
                            "Status": "string",
                            "User": "string"
                        };
